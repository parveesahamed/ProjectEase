...
export default function CombinedAuth() {
    ...
    const handleSignUpSubmit = async (e) => {
        ...
        setErr("");
        setLoading(true);
        console.log("New User Details:", { name: signUpForm.name, email: signUpForm.email, role: signUpForm.role });
        try {
    ...
    const handleGoogleLogin = async () => {
        ...
        try {
            ...
            console.log("New Google User:", { name: userCred.user.displayName, email: userCred.user.email, role: 'User' });
            handleAuthResponse(userCred);
        } catch (error) {
            handleAuthError(error);
        } finally {
    ...
    return (
        <div className="flex items-center justify-center min-h-screen bg-[#111827] p-4">
                    ...
                    <div className="bg-[#1f2937] p-8 rounded-2xl shadow-lg">
                        <form onSubmit={handleSignUpSubmit} className="flex flex-col items-center text-center">
                            ...
                            <div className="relative w-full my-1.5">
                                <select name="role" value={signUpForm.role} onChange={handleSignUpChange} className="bg-[#374151] border-none p-3 w-full text-white rounded appearance-none cursor-pointer">
                            ...
                            <button type="submit" disabled={loading} className="rounded-full border border-transparent bg-gradient-to-r from-blue-600 to-purple-600 text-white text-sm font-bold py-3 px-12 tracking-wider uppercase mt-4">
                                {loading ? 'Creating...' : 'Sign Up'}
                            </button>
                             <p className="text-gray-400 mt-6 text-sm">Already have an account?</p>
                             <button type="button" onClick={handleSignInClick} className="text-white font-bold mt-2 underline">Sign In</button>
                        </form>
                    </div>
                ) : (
                    <div className="bg-[#1f2937] p-8 rounded-2xl shadow-lg">
                        <form onSubmit={handleSignInSubmit} className="flex flex-col items-center text-center">
                            ...
                            <button type="submit" disabled={loading} className="rounded-full border border-transparent bg-gradient-to-r from-blue-600 to-purple-600 text-white text-sm font-bold py-3 px-12 tracking-wider uppercase">
                            ...
                            <button type="button" onClick={handleGoogleLogin} disabled={loading} className="w-full flex items-center justify-center gap-2 bg-white text-black py-2.5 rounded-lg hover:bg-gray-200 transition font-semibold">
                                <GoogleIcon /> Continue with Google
                            </button>
                            <p className="text-gray-400 mt-6 text-sm">Don't have an account?</p>
                            <button type="button" onClick={handleSignUpClick} className="text-white font-bold mt-2 underline">Sign Up</button>
                        </form>
                    </div>
                )}
            </div>
            ...
            <div className={`relative overflow-hidden w-full max-w-4xl min-h-[620px] rounded-lg bg-[#1f2937] shadow-lg hidden md:block`}>
                ...
                {/* Sign In Form */}
                <div className={`absolute top-0 h-full left-0 w-1/2 z-20 transition-all duration-600 ease-in-out ${isSignUpActive ? 'transform translate-x-full' : ''}`}>
